...
export class FirecrackerService {
  ...
  constructor(
    ...
    public readonly pathToSocket: string,
  ) {
  ...
  getVMLogsPath(instanceId: string): string {
    return `/tmp/${instanceId}.log`;
  }
  ...
  startFirecrackerInstance(filesPrefix: string): number {
    ...
    const logPath = this.getVMLogsPath(filesPrefix);
  ...
  async withVM(
    ...
    const vmId = config.vmId ?? uuidv4();
    const kernelPath = config.kernelPath ?? "/hocus-resources/vmlinux-5.6-x86_64.bin";
    const socketPath = `/tmp/${vmId}.sock`;
    const fcPid = this.startFirecrackerInstance(socketPath);
    ...
    try {
      ...
      const vmConfig = await this.createVM({
        ...
        tapDeviceName: this.tapDeviceName,
        tapDeviceCidr: this.tapDeviceCidr,
        extraDrives: config.extraDrives ?? [],
      });
      ...
      await withSsh(config.ssh, async (ssh) => {
        ...
        await watchFileUntilLineMatches(/reboot: System halted/, this.getVMLogsPath(vmId), 10000);
      });
    } finally {
      process.kill(fcPid);
