...
RUN apt-get update \
    ...
    && add-apt-repository "deb http://httpredir.debian.org/debian sid main" \
    && apt-get update \
    && apt-get -t sid install -y qemu-system skopeo net-tools iproute2 iptables psmisc socat openssh-server iputils-ping tmux libaio1 libnl-3-200 libnl-genl-3-200
